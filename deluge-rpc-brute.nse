...
portrule = shortport.port_or_service(58846, "", "tcp", "open")
...
Driver = {
  ...
  login = function(self, username, password)
    ...
    local status, err, msq
    local request = rencoded_login_request(username, password)
    status, err = self.socket:send(zlib.compress(request))
